...
namespace ManagedComponent.MvOZCOT
{
    ...
    public class mvOZCOTClass : TBaseComp
    {
        ...
        private static String _SVERSION = "2.10";
        ...
        //TODO: ??  should these events be published? What conditions act as triggers?  
        ...
        private const int PROP_smi30 = PROP_START_INDEX + 141;
        private const int PROP_wli = PROP_START_INDEX + 142;
        private const int PROP_evap_plant = PROP_START_INDEX + 143;
        private const int PROP_evap_soil = PROP_START_INDEX + 144;
        private const int PROP_evap_pot = PROP_START_INDEX + 145;
        private const int PROP_evap_tot = PROP_START_INDEX + 146;
        private const int PROP_bolls_sc = PROP_START_INDEX + 147;
        private const int PROP_nuptake = PROP_START_INDEX + 148;
        private const int PROP_squarz_max = PROP_START_INDEX + 149;
        private const int PROP_lai_max = PROP_START_INDEX + 150;
        private const int PROP_defol_das = PROP_START_INDEX + 151;
        private const int PROP_defol2_das = PROP_START_INDEX + 152;
        private const int PROP_height = PROP_START_INDEX + 153;
        private const int PROP_carcap_n = PROP_START_INDEX + 154;
        private const int PROP_lint_bales = PROP_START_INDEX + 155;
        private const int PROP_lint_kg = PROP_START_INDEX + 156;
        private const int PROP_background_retention = PROP_START_INDEX + 157;

        private const int PROP_cultivar_default = PROP_START_INDEX + 158;
        private const int PROP_cultivars_list = PROP_START_INDEX + 159;
        private const int PROP_n_uptake = PROP_START_INDEX + 160;
        private const int PROP_ep_cm = PROP_START_INDEX + 161;
        private const int PROP_plant_status = PROP_START_INDEX + 162;

        private const int PROP_cottonCO2 = PROP_START_INDEX + 163;

        private const int PROP_rootLACrit = PROP_START_INDEX + 164;
        //private const int PROP_  unused    = PROP_START_INDEX + 165;
        private const int PROP_rootDepth_mm = PROP_START_INDEX + 166;
        private const int PROP_RFV_max = PROP_START_INDEX + 167;
        private const int PROP_specific_root_length = PROP_START_INDEX + 168;
        ...
        private const int PROP_rlv = PROP_START_INDEX + 183;                  //root length volume (mm/mm3) for interface to SWIM

        private const int PROP_useDDsumTo1stSq = PROP_START_INDEX + 184;           //flag to use sumDDto1stSq = true (default), or RateOfDevel = false 
       ...
       //                                                                       currently cannot pass 2D arrays to Manager2 
        ...
        private const int PROP_useYeatesBPerFunct = PROP_START_INDEX + 195;    //flag to use Yeates (Ord) Boll Period function = true, or OZCOT (Hearn) standard function = false (default)

        private const int PROP_x_sw_ratio = PROP_START_INDEX + 200;          //soil water fraction (avail/total)
        private const int PROP_y_swfac_root = PROP_START_INDEX + 201;        //sw factor for root growth
        private const int PROP_x_sat_ratio = PROP_START_INDEX + 202;         //saturated soil water fraction above DUL
        private const int PROP_y_satfac_root = PROP_START_INDEX + 203;       //saturated factor for root growth
        private const int PROP_x_sw_ds_ratio = PROP_START_INDEX + 204;       //soil water supply/demand ratio
        private const int PROP_y_sw_ds_fac_root = PROP_START_INDEX + 205;    //sw supply/demand factor for root growth
        ...
        private const double sm2smm = 1000000.0;        //converting square metres to square mm
        ...
        private const double ozcot_kvalue = 1.0;  // for light interception using Beer's law
        ...
        // default values for root growth and soil conditions
        // these are used if no user input values are found
        // (default values for array type properties cannot be defined in the 'addProperty' section)

        private readonly double[] x_sw_ratio_defaults = {0.0, 0.25, 0.5, 1.0 };              // soil water fraction (avail/total)
        private readonly double[] y_swfac_root_defaults = { 0.0, 0.2, 1.0, 1.0 };            // sw factor for root growth

        private readonly double[] x_sat_ratio_defaults = { 1.0, 1.2 };                       // saturated soil water fraction above DUL
        private readonly double[] y_satfac_root_defaults = { 1.0, 0.0 };                     // saturated factor for root growth

        private readonly double[] x_sw_ds_ratio_defaults = { 0.0, 0.3, 1.0 };                // soil water supply/demand ratio
        private readonly double[] y_sw_ds_fac_root_defaults = { 0.3, 1.0, 1.0 };             // sw supply/demand factor for root growth
        ...
        private double[] cll = new double[max_layers + 1];  //array of soil water Crop Lower Limit (CLL) values 

        private int num_kl_vals;
        private double[] kl = new double[max_layers + 1];  //array of soil water crop 'kl' values 

        private int num_xf_vals;
        private double[] xf = new double[max_layers + 1];  //array of soil water crop 'xf' values 
        ...
        // private int rootExpansionDelay = 33;    // delay in days from sowing until roots begin rapid expansion (default for Narrabri = 33)
        ...
        private bool useYeatesBPerFunct;        // flag to use Yeates (Ord) Boll Period function
        ...
        private double vpd;
        private double bper;
        private double[] dlayr = new double[max_layers + 1];     //in mm
        private double[] dlayr_cm = new double[max_layers + 1];  //in cm
        ...
        private double rootDepth_mm;
        private double dltRootDepth_mm;
        private double rootGrowthRateDD;
        private double RFV_max;     //root front velocity (root growth rate max) mm/day
        private double[] root_length = new double[max_layers + 1]; // root length array (root length by layer)
        private double[] rlv = new double[max_layers + 1];         // rlv array (root volume by layer)
        private int specific_root_length;  // specific root lenght <units="mm/g"description="specific length of roots - who knows?">60000</specific_root_length>
        private double initial_root_wt;
        private double rootLACrit;  //(root) leaf area critical value for start of descent of extraction front
        private double rootDepthRate;
        private double storedSWDemandSupplyRatio;  //stored value of soil water demend/supply for use in root growth rate calc
        ...
        private const double root_res_n_conc = 0.005;     // N concentration of root residue
        ...
        private const double stem_res_n_conc = 0.005;     // N concentration of stem residue 
        //private const double pod_res_n_conc = 0.015;    // avg N concentration of squares and green bolls (small)
        private const double pod_res_n_conc = 0.014;      // avg N concentration of squares and green bolls (small)
        private const double meal_res_n_conc = 0.03;      // N concentration of cotton seed 
        private const double lint_res_n_conc = 0.015;     // N concentration of lint 
       ...
       // private double pod_res_n;
        ...
        //Root growth critical values (for establishing growth factors based on interpolated values)
        // note: these are init properties, but default values for array types cannot be specified in 'addProperty'
        //       values may be read in from user input, or may default to the defined default values eg. x_sw_ratio_defaults
        private double[] x_sw_ratio = { };            // soil water fraction (avail/total)
        private double[] y_swfac_root = { };          // sw factor for root growth

        private double[] x_sat_ratio = { };           // saturated soil water fraction above DUL
        private double[] y_satfac_root = { };         // saturated factor for root growth

        private double[] x_sw_ds_ratio = { };         // soil water supply/demand ratio
        private double[] y_sw_ds_fac_root = { };      // sw supply/demand factor for root growth

        //Defoliation parameters
        private double defoliationDDSum = 0.0;
        private int defoliationDayCnt = 0;

        //for SteveY NAust studies
        // array of last 3 days minimum temperatures
        // private FixedSizedQueue<double> minTemps = new FixedSizedQueue<double>(3);
        private Queue<double> minTemps = new Queue<double>(3);
        private double minTempAvg3Days = 0.0;
        // list of cohorts that flower on given DAS day
        Dictionary<int, List<int>> floweringDays = new Dictionary<int, List<int>>();
            ...
            : base(compID, parentCompID, messageCallback, _STYPE, _SVERSION, _SAUTHOR)
        {
            ...
            setPropertyRange(PROP_leaf_res_n_conc, 0.015, 0.0, 0.05);
            ...
            addProperty("rtdep_max", PROP_rtdep_max, true, false, true, "cm", false, "double", "maximum depth of rooting", "maximum depth of rooting - defaults to profile depth");
            ...
            // root expansion delay based on root critical leaf area
            addProperty("rootLACrit", PROP_rootLACrit, true, false, true, "m2/plant", false, "double", "critical leaf area per plant limiting root expansion", "small leaf area per plant after sowing until roots rapidly expand - default = 0.012 m2 ");
            setPropertyRange(PROP_rootLACrit, 0.012, 0.001, 0.1);
            // max root growth rate
            addProperty("RFV_max", PROP_RFV_max, true, false, true, "mm/day", false, "integer4", "max rate of root growth", "max root front velocity default = 30 mm/day ");
            setPropertyRange(PROP_RFV_max, 30, 5, 60);
            // specific root length 
            addProperty("specific_root_length", PROP_specific_root_length, true, false, true, "mm/g", false, "integer4", "specific root length per unit wt", "root length per unit wt: default = 60000 mm/g ");
            setPropertyRange(PROP_specific_root_length, 60000, 20000, 180000);

            // sw ratio for root growth (array of x values)
            addProperty("x_sw_ratio", PROP_x_sw_ratio, false, false, true, "-", true, "double", "fasw", "fraction avail soil water");
            setPropertyRange(PROP_x_sw_ratio, 0.0, 0.0, 1.0);
            // sw factor for root growth (array of y values)
            addProperty("y_swfac_root", PROP_y_swfac_root, false, false, true, "-", true, "double", "sw factor root growth", "sw factor root growth");
            setPropertyRange(PROP_y_swfac_root, 1.0, 0.0, 1.0);

            // saturated ratio for root growth (array of x values)
            addProperty("x_sat_ratio", PROP_x_sat_ratio, false, false, true, "-", true, "double", "fasw above DUL", "fraction avail soil water DUL to SAT");
            setPropertyRange(PROP_x_sat_ratio, 1.0, 1.0, 2.0);
            // saturated factor for root growth (array of y values)
            addProperty("y_satfac_root", PROP_y_satfac_root, false, false, true, "-", true, "double", "saturated sw factor root growth", "saturated sw factor root growth");
            setPropertyRange(PROP_y_satfac_root, 1.0, 0.0, 1.0);

            // soil water supply/demand ratio (array of x values)
            addProperty("x_sw_ds_ratio", PROP_x_sw_ds_ratio, false, false, true, "-", true, "double", "sw supply/demand ratio", "sw supply/demand ratio");
            setPropertyRange(PROP_x_sw_ds_ratio, 0.0, 0.0, 1.0);
            // sw supply/demand factor for root growth (array of y values)
            addProperty("y_sw_ds_fac_root", PROP_y_sw_ds_fac_root, false, false, true, "-", true, "double", "sw supply/demand factor root growth", "sw supply/demand factor root growth");
            setPropertyRange(PROP_y_sw_ds_fac_root, 0.0, 0.0, 1.0);


            // use sum of DayDegrees to 1st Square flag
            addProperty("useDDsumTo1stSq", PROP_useDDsumTo1stSq, true, false, true, "-", false, "boolean", "use DayDegree sum for 1st Square", "use DayDegree sum to predict 1st Square. default = true ie use DayDegree sum");
             setPropertyRange(PROP_useDDsumTo1stSq, 1,0,1);

            // use Yeates (Ord) Boll Period function flag
            addProperty("useYeatesBPerFunction", PROP_useYeatesBPerFunct, false, false, true, "-", false, "boolean", "use Yeates (Ord) Boll Period function", "use Yeates (Ord) Boll Period function. default = false ie use Standard OZCOT Hearn function");
            setPropertyRange(PROP_useYeatesBPerFunct, 0, 0, 1);
            ...
            addProperty("rootDepth_mm", PROP_rootDepth_mm, true, false, false, "mm", false, "double", "rooting depth mm", "rooting depth mm");
            ...
            addProperty("smi30", PROP_smi30, true, false, false, "-", false, "double", "SMI of top 30cm", "soil moisture index of top 30cm");
            ...
            // disable 'rlv' until it can be fully validated - initial coding complete; unit conversion and parameter values need confirming
            //addProperty("rlv", PROP_rlv, true, false, false, "mm/mm3", true, "double", "rootLengthVolume", "rootLengthVolume by soil layer");
        ...
        public override void initProperty(int iPropertyID, TTypedValue aValue)
        {
            ...
            switch (iPropertyID)
            {
                ...
                case PROP_ll: 
                    cll = new double[aValue.count() + 1];
                    cll[0] = 0;
                    for (i = 1; i <= aValue.count(); i++)
                    {
                        cll[i] = aValue.item(i).asDouble();
                    }
                ...
                case PROP_rootLACrit: rootLACrit = aValue.asDouble();
                    break;
                ...
                case PROP_RFV_max:
                    RFV_max = aValue.asInt();
                    break;

                case PROP_specific_root_length:
                    specific_root_length = aValue.asInt();
                    break;


                case PROP_useYeatesBPerFunct:
                    useYeatesBPerFunct = aValue.asBoolean();
                    break;



                case PROP_x_sw_ratio:
                    x_sw_ratio = new double[aValue.count()];
                    for (i = 0; i < aValue.count(); i++)
                    {
                        x_sw_ratio[i] = aValue.item(i+1).asDouble();  //tTyped arrays are one based 
                    }
                    break;

                case PROP_y_swfac_root:
                    y_swfac_root = new double[aValue.count()];
                    for (i = 0; i < aValue.count(); i++)
                    {
                        y_swfac_root[i] = aValue.item(i + 1).asDouble();
                    }
                    break;


                case PROP_x_sat_ratio:
                    x_sat_ratio = new double[aValue.count()];
                    for (i = 0; i < aValue.count(); i++)
                    {
                        x_sat_ratio[i] = aValue.item(i + 1).asDouble();
                    }
                    break;

                case PROP_y_satfac_root:
                    y_satfac_root = new double[aValue.count()];
                    for (i = 0; i < aValue.count(); i++)
                    {
                        y_satfac_root[i] = aValue.item(i + 1).asDouble();
                    }
                    break;


                case PROP_x_sw_ds_ratio:
                    x_sw_ds_ratio = new double[aValue.count()];
                    for (i = 0; i < aValue.count(); i++)
                    {
                        x_sw_ds_ratio[i] = aValue.item(i + 1).asDouble();
                    }
                    break;

                case PROP_y_sw_ds_fac_root:
                    y_sw_ds_fac_root = new double[aValue.count()];
                    for (i = 0; i < aValue.count(); i++)
                    {
                        y_sw_ds_fac_root[i] = aValue.item(i + 1).asDouble();
                    }
                    break;
        ...
        public override bool writeProperty(int iPropertyID, TTypedValue aValue)
        {
            ...
            switch (iPropertyID)
            {
               case PROP_crop_type: crop_type = aValue.asStr();
                ...
                case PROP_ll: //cll = aValue.asDouble();
                    cll = new double[aValue.count() + 1];
                    cll[0] = 0;
                    for (i = 1; i <= aValue.count(); i++)
                    {
        ...
        public override void readProperty(int iPropertyID, uint iRequestor, ref TPropertyInfo aValue)
        {
            ...
            switch (iPropertyID)
            {
                ...
                case PROP_ll: //cll
                    aValue.setElementCount((uint)cll.Length - 1);
                    for (i = 1; i < cll.Length; i++)
                    {
                        aValue.item(i).setValue(cll[i]);
                    }
                    break;
                ...
                case PROP_dm_green:  // (g/m2)
                    Array.Clear(dm_crop, 0, dm_crop.Length);   // 1 based array  
                    dm_crop[root] = dm_green[root];  // g/m2
                ...
                case PROP_biomass:
                    double dm_all = 0.0;
                    dm_all = (dm_green.Sum() - dm_green[root]) + dm_senesced.Sum()  + dm_harvested.Sum();
                ...
                case PROP_rootDepth_mm:
                    aValue.setValue(rootDepth_mm);
                    break;

                //case PROP_rootFeedback:
                //    aValue.setValue(root_feedback);
                //    break;
                ...
                case PROP_ozcot_rwu: 
                    //sv- 2015_04_01 needed because ep does not equal root water uptake summed over profile. (THIS NEEDS TO BE FIXED) - Done March 2016
                ...
                // // NOTE: 'rlv' reporting currently disabled until proper validation can be undertaken
                //case PROP_rlv:
                //    //rootLengthVolume by layer :  for interface to SWIM modules

                //    aValue.setElementCount((uint)rlv.Length - 1);
                //    for (i = 1; i < rlv.Length; i++)
                //    {
                //        aValue.item(i).setValue(rlv[i]);
                //    }
                //    break;
                ...
                case PROP_rootLACrit:
                    aValue.setValue(rootLACrit);
                    break;

                case PROP_RFV_max:
                    aValue.setValue(RFV_max);
                    break;

                case PROP_specific_root_length:
                    aValue.setValue(specific_root_length);
                    break;
        ...
        public void ozcotGetInitDrivers(int iEventID)
        {
            ...
            if (initDataNode != null)
            {
                ...
                XmlNode llNode = FSDMLComp.firstElementChild(initDataNode, "ll");
                if (llNode != null)
                {
                    string vals = llNode.InnerText;
                    ...
                    cll = new double[DoubleValues.Length + 1];
                    cll[0] = 0;
                    for (int i = 0; i < DoubleValues.Length; i++)
                    {
                        cll[i + 1] = DoubleValues[i];
                    }
                    num_ll_vals = DoubleValues.Length;
                }

                // Crop 'kl' values for the soil
                //Console.WriteLine(" Looking for kL node in XML for Cotton ");
                XmlNode klNode = FSDMLComp.firstElementChild(initDataNode, "kl");
                if (klNode != null)
                {
                    //Console.WriteLine(" Found kL node in XML ");  
                    string vals = klNode.InnerText;
                    string[] Values = vals.Split(null as char[], StringSplitOptions.RemoveEmptyEntries);
                    double[] DoubleValues = MathUtility.StringsToDoubles(Values);
                    kl = new double[DoubleValues.Length + 1];
                    kl[0] = 0;
                    for (int i = 0; i < DoubleValues.Length; i++)
                    {
                        kl[i + 1] = DoubleValues[i];
                    }
                    num_kl_vals = DoubleValues.Length;

                    //Console.WriteLine("  Number of kl values: {0} ", num_kl_vals);
                    //Console.WriteLine(" kL : {0} , {1} , {2} {3} {4} {5}",kl[0], kl[1], kl[2], kl[3], kl[4], kl[5]);
                    //Console.WriteLine("  \n   ");
                }

                // Crop Lower Limit values for the soil
                XmlNode xfNode = FSDMLComp.firstElementChild(initDataNode, "xf");
                if (xfNode != null)
                {
                    string vals = xfNode.InnerText;
                    string[] Values = vals.Split(null as char[], StringSplitOptions.RemoveEmptyEntries);
                    double[] DoubleValues = MathUtility.StringsToDoubles(Values);
                    xf = new double[DoubleValues.Length + 1];
                    xf[0] = 0;
                    for (int i = 0; i < DoubleValues.Length; i++)
                    {
                        xf[i + 1] = DoubleValues[i];
                    }
                    num_xf_vals = DoubleValues.Length;
                }
            ...
            for (layer = 1; layer <= nlayr; layer++)
            {
                ...
                sat_adj[layer] = (dul_layers[layer] - cll[layer]) / 0.87 + cll[layer];   // adjust to match ozcot soil characterisation
                sat_adj[layer] = Math.Min(sat_adj[layer], sat_layers[layer]);
                ullayr[layer] = sat_adj[layer] - cll[layer];   // upper limit of profile
                profileDepth = profileDepth + dlayr_cm[layer];				// depth of profile
                ul = ul + ullayr[layer] * dlayr_cm[layer];		// upper limit for profile
                wpwc = wpwc + cll[layer] * dlayr_cm[layer];
            }
        ...
        public void ozcot_get_drivers_daily(int iEventID)
        {
            ...
            // store minimum temperature in '3 day' array for use in testing fruit shedding (SteveY NAust studies)
            minTemps.Enqueue(tempmn);
            if (minTemps.Count > 3) minTemps.Dequeue();
            minTempAvg3Days = minTemps.Average();
            ...
            for (layer = 1; layer <= nlayr; layer++)
            {
                ...
                swlayr[layer] = swlayr[layer] - cll[layer];
        ...
        public void ozcot_ResetVarsFlags()
        {
           ...
           // availn = 0.0;
            ...
            //variables used for SteveY NAust studies
            minTemps.Clear();
            floweringDays.Clear();
        ...
        public void ozcot_report_cultivar_params()
        {
            ...
            Console.WriteLine("         flai       =  {0}", flai.ToString("##0.00"));
        ...
        public void ozcot_sow()
        {
            ...
            pp = ppm2_target * effectiveRowSpacing;    
            ...
            {
                Console.WriteLine("             {0}          {1} ", dlayr_cm[i].ToString("#0.0"), cll[i].ToString("0.000"));
            }
        ...
        public void ozcot_end_crop()
        {
            ...
            if (crop_in)
            {
                ...
                if (dlt_dm_crop[root] > 0.0)   //only do this if there are roots to incorporate
                {
                    ...
                    deepest_layer = mvDataFunctions.find_layer_no(rtdep, dlayr_cm, max_layers);
        ...
        public void ozcot_crop_root_length_distrib(ref double[] root_array, double root_sum)
        {
            ...
            {
                cum_depth = cum_depth + dlayr_cm[layer];
            ...
            //Console.WriteLine("DAS= {0} deepest_layer= {1}  ", DAS, deepest_layer);
            //Console.WriteLine("DAS= {0} root_distrb= {1}   {2}  {3}  {4}  {5}  {6}  {7}", DAS, root_distrb[1].ToString("#.####"), root_distrb[2].ToString("#.####"), root_distrb[3].ToString("#.####"), root_distrb[4].ToString("#.####"), root_distrb[5].ToString("#.####"), root_distrb[6].ToString("#.####"), root_distrb[7].ToString("#.####"));
            //Console.WriteLine("DAS= {0} root_array= {1}   {2}  {3}  {4}  {5}  {6}  {7}", DAS, root_array[1].ToString("#.####"), root_array[2].ToString("#.####"), root_array[3].ToString("#.####"), root_array[4].ToString("#.####"), root_array[5].ToString("#.####"), root_array[6].ToString("#.####"), root_array[7].ToString("#.####"));
        ...
        public void ozcot_pltgrw()
		{
            ...
            if (cropStage == harvest_ripe)
            {
                ...
                return;                      // do not continue to grow the crop
            }
            ...
            } // end 'if emerged check if dead'

            //--- if crop is dead, don't grow the crop any further
            if (plant_status == status_dead) return;
            ...
            //--new code
            if (iemrg > 0 && iemrg < DAS) ozcot_root_depth();
            ...
            ////debug version 
            //rtdep = rootDepth_mm / 10;


            //----- increase root length per layer (rlv or rld) ----------------------------------------------------
            if (iemrg > 0 && iemrg < DAS) ozcot_root_length_growth();
        ...
        // ====================================================================
        public void ozcot_root_depth()
        {
            // ====================================================================
            //  Calculate change in plant rooting depth

            //- implementation section ----------------------------------
            // NOTE: Cotton(OZCOT) uses soil arrays, zero based but [0] is empty, so layer number is logical number
            // root depth was 'rtdep',  for testing it is 'rootDepth_mm'
            int layer;
            int nextLayer;
            double cumLayerDepth;
            double cumLayerDepth_xf;
            double rootdepthInLayer;
            double rootedFractionLayer;
            double leafAreaPlant;
            double sw_DS_Factor;
            double rootBLR;
            double rootAssimFactor;  // 1-(bload/cutout)  where cutout = carcap_c * fcutout
            double leafAreaFactor;
            double fasw, fasw1, fasw2;  //fraction of plant available soil water
            double swAvailFactor, swsmiFactor, swwliFactor;  //factors that reflect the availablily of esw below the rooting depth.
            //double[] x_sw_ratio;
            //double[] y_swfac_root;
            //double[] x_sat_ratio;
            //double[] y_satfac_root;
            //double[] x_sw_ds_ratio;
            //double[] y_sw_ds_fac_root;



            // rootDepthRate = rootDepthRatePot (mm/day) init variable, default = 30 mm/day,  could add varietal adj factor
            rootDepthRate = 30.0;  // default value (mm/day)
            if (RFV_max > 0.0)  rootDepthRate = RFV_max;   //RFV_max is user input value - use this if it is specified

            // ensure values have been set for (x,y) value pairs (user input or default values need to be set)
            if (x_sw_ratio.Count() < 1)
            {
                x_sw_ratio = x_sw_ratio_defaults;
                y_swfac_root = y_swfac_root_defaults;
            }

            if (x_sat_ratio.Count() < 1)
            {
                x_sat_ratio = x_sat_ratio_defaults;
                y_satfac_root = y_satfac_root_defaults;
            }

            if (x_sw_ds_ratio.Count() < 1)
            {
                x_sw_ds_ratio = x_sw_ds_ratio_defaults;
                y_sw_ds_fac_root = y_sw_ds_fac_root_defaults;
            }


            
            // determine the layer with the root front
            cumLayerDepth = 0.0;
            layer = 0;
            for (int idx = 1; idx <= nlayr; idx++)    
            {
                if ((cumLayerDepth + dlayr[idx]) >= rootDepth_mm)
                {
                    layer = idx;
                    break;
                }
                cumLayerDepth += dlayr[idx];    //depth to bottom of layer before root front in mm
                if (idx == nlayr) layer = idx;  // at the limit of the profile depth
            }
            
            // determine root depth within the layer (mm)
            rootdepthInLayer = rootDepth_mm - cumLayerDepth;
            rootdepthInLayer = Math.Min(dlayr[layer], Math.Max(0.0, rootdepthInLayer));  // set limits (0.0 <= rootdetphInLayer <= depthOfLayer)
            // determine fraction of layer depth with roots
            rootedFractionLayer = 1.0;
            if (dlayr[layer] > 0.0) rootedFractionLayer = rootdepthInLayer / dlayr[layer];

            // Console.WriteLine("DAS= {0} RootDepth_mm= {1} RootingLayer= {2} cumDepthToLayer= {3} rootsInLayer= {4} dlayr[1]= {5} dlayr[2]= {6}", DAS, rootDepth_mm.ToString("###.#"), layer, cumLayerDepth, rootdepthInLayer.ToString("###.#"), dlayr[1],dlayr[2]);

            // calc soil water availability for 'smi' type value
            // unrooted portion of current layer, plus weighted portion of lower layer if not at profile limit
            // deepest soil layer (as a max for layer tests) is nlayr
            nextLayer = Math.Min(layer + 1,nlayr);
            fasw1 = swlayr[layer]*dlayr[layer] / ((dul_layers[layer] - cll[layer])*dlayr[layer]);
            fasw1 = Math.Min(1.0, Math.Max(0.0, fasw1));               // set limits (0.0 <= fasw1 <= 1.0)

            fasw2 = swlayr[nextLayer] * dlayr[nextLayer] / ((dul_layers[nextLayer] - cll[nextLayer]) * dlayr[nextLayer]);
            fasw2 = Math.Min(1.0, Math.Max(0.0, fasw2));              // set limits (0.0 <= fasw2 <= 1.0)

            fasw = (fasw2 * rootedFractionLayer) + (fasw1 * (1 - rootedFractionLayer));

            // calc swsmiFactor based on fasw for the soil below the rooting front  
            swsmiFactor = 1.0;
            //x_sw_ratio = new double [4] { 0.0, 0.25, 0.5, 1.0};
            //y_swfac_root = new double[4] { 0.0, 0.2, 1.0, 1.0 };
            swsmiFactor = mvDataFunctions.linear_interp_real(fasw, x_sw_ratio, y_swfac_root, x_sw_ratio.Count());

            // calc swwliFactor if fasw1 is > 1  ie. if above dul
            swwliFactor = 1.0;
            if (fasw1 > 1.0)  //saturated layer
            {
                //x_sat_ratio = new double[2] { 1.0, 1.2 };
                //y_satfac_root = new double[2] {  1.0, 0.0 };
                swwliFactor = mvDataFunctions.linear_interp_real(fasw1, x_sat_ratio, y_satfac_root, x_sat_ratio.Count());
            }

            swAvailFactor = Math.Min(swsmiFactor, swwliFactor);  //only one factor can be valued < 1 at a time

            //Console.WriteLine("DAS= {0} fasw1= {1} fasw2= {2} fasw= {3} rootedFractionLayer= {4} swsmiFactor= {5} swwliFactor={6} swAvailFactor= {7} dltRootDepth_mm= {8} rootDepth= {9} ", DAS, fasw1.ToString("0.##"), fasw2.ToString("0.##"), fasw.ToString("0.##"), rootedFractionLayer.ToString("0.###"), swsmiFactor.ToString("0.##"), swwliFactor.ToString("0.###"), swAvailFactor.ToString("0.###"), dltRootDepth_mm.ToString("0.###"), rootDepth_mm.ToString("##0."));

            // use the sw demand by the plant and the sw supply available for the previous day
            //  (sw demand/supply ratio affects the 'need' of the plant to grow roots)
            //  calculated in ozcot_removeEp_swbal calculations.  Yesterday's value.  storedSWDemandSupplyRatio
            //x_sw_ds_ratio = new double[3] { 0.0, 0.3, 1.0 };
            //y_sw_ds_fac_root = new double[3] { 0.3, 1.0, 1.0 };
            sw_DS_Factor = mvDataFunctions.linear_interp_real(storedSWDemandSupplyRatio, x_sw_ds_ratio, y_sw_ds_fac_root, x_sw_ds_ratio.Count());

            //  
            //   Temperature Effect On Root Advance
            //   Note: this is covered by the assimilate supply for root growth as temperatures (both high and low) directly affect plant growth.
            //         Temperature effect is not included separately.
            //


            // For very low leaf area, limit root growth.  
            //  Primarily affects the observed delay to the onset of the descent of the extraction front during early growth
            leafAreaPlant = laiField / ppm2;
            //leafAreaFactor = leafAreaPlant / 0.012;    //range 0.001 - 0.02  default=0.012 for Myall Vale    externalise 'rootLACrit'
            leafAreaFactor = leafAreaPlant / rootLACrit;    //range 0.001 - 0.02  default=0.012 for Myall Vale    externalise 'rootLACrit'
            leafAreaFactor = Math.Min(Math.Max(leafAreaFactor, 0.1),1.0);

            // determine the assimilate supply factor for root growth
            rootBLR = 1.0;
            rootAssimFactor = 1.0;
            if ((bload / cutout) > 0.0)
            {
                rootBLR = (bload / cutout);
                rootAssimFactor = -0.12 * Math.Log(rootBLR) + 0.03;            //main driving function !!
                //rootAssimFactor = -0.144 * Math.Log(rootBLR) + 0.07;
            }
            rootAssimFactor = Math.Min(Math.Max(rootAssimFactor, 0.0), 1.0);


            
            // estimate the root growth

            sw_DS_Factor = 1.0;

            dltRootDepth_mm = 0.0;
            dltRootDepth_mm = rootDepthRate * Math.Min(sw_DS_Factor, swAvailFactor) * xf[layer] * Math.Min(rootAssimFactor, leafAreaFactor) ;
            dltRootDepth_mm = Math.Max(0.0, dltRootDepth_mm);  // no negative root growth
            rootDepth_mm += dltRootDepth_mm;

            // limit root depth if entering layer where xf == 0.0
            cumLayerDepth_xf = 0.0;
            for (int idx = 1; idx < xf.Count(); idx++)
            {
                if ((xf[idx]) <= 0.0)
                {
                    break;
                }
                cumLayerDepth_xf += dlayr[idx];         //depth to bottom of layer before root front
            }

            // limit root depth to Minimum of (max rooting depth  |  depth of soil profile  |  layer with xf = 0.0)
            rootDepth_mm = Math.Min(rootDepth_mm, Math.Min(cumLayerDepth_xf, Math.Min((rtdep_max * 10), (profileDepth * 10))));


            //Console.WriteLine("DAS= {0} smiFactor= {1} wliFactor= {2} swAvailFactor= {3} storedSWDSRatio= {4} sw_DS_Factor= {5} xf[layer]={6} rootAssimFactor= {7} leafAreaPlant= {8} dltRootDepth_mm= {9} rootDepth= {10} rtRate= {11} ", DAS, swsmiFactor.ToString("0.##"), swwliFactor.ToString("0.##"), swAvailFactor.ToString("0.##"),  storedSWDemandSupplyRatio.ToString("0.###"), sw_DS_Factor.ToString("0.##"), xf[layer],rootAssimFactor.ToString("0.##"), leafAreaPlant.ToString("0.####"), dltRootDepth_mm.ToString("0.###"), rootDepth_mm.ToString("##0."), rootDepthRate);
            //Console.WriteLine("DAS= {0} profileDepth= {1} rootDepth_mm= {2} cumLayer_xf= {3} xf_count= {4}", DAS, profileDepth, rootDepth_mm,cumLayerDepth_xf, xf.Count());
            //Console.WriteLine("DAS= {0} bload/cutout= {1} rootAssimFactor= {2}", DAS,  (bload / cutout).ToString("0.###"), rootAssimFactor.ToString("0.###"));

            // set value of rtdep in cm
            rtdep = rootDepth_mm / 10;
        ...
        // ====================================================================
        public void ozcot_root_length_growth()
        {
            // ====================================================================
            ...
            double rld;
            double rldPlant;
            double rlvFactorTotal;
            double branchingFactor;
            double dltLenghtTotal;
            double swFactor;
            double fasw;


            double[] dltRootLength = new double[nlayr];  //max number of soil layers defined in the profile
            Array.Clear(dltRootLength,0, nlayr);
            double[] rlvFactor = new double[nlayr];  //max number of soil layers defined in the profile
            Array.Clear(rlvFactor, 0, nlayr);
            rlvFactorTotal = 0.0;

            int deepest_layer = (mvDataFunctions.find_layer_no(rtdep,dlayr_cm,nlayr));

            for (int layer = 1; layer <= deepest_layer; layer++)
            {
                rld = (root_length[layer] / dlayr[layer]);

                rldPlant = (rld / ppm2);

                //branchingFactor = rel_root_rate.value(rldPlant);  //branching factor? (who knows?) based on crop type and plant population density and rld
                branchingFactor = 1.0;                              // range 0 - 1,  default is 1.0 

                fasw = sw_avail_ratio(layer);                       // fraction of available sw (0=dry, 1=DUL, >1 saturated)
                swFactor = 1.0;
                swFactor = Math.Min(sw_fac_root(fasw), sat_fac_root(fasw));               // either dry soil or saturated soil might be limiting root growth

                // combined factor indicating how hospitable the soil layer is for root growth
                rlvFactor[layer] = (swFactor *                                            // soil water availability in layer factor
                                    branchingFactor *                                     // branching factor
                                    (xf[layer] *  (dlayr_cm[layer] / rtdep)));            // effect of layer restrictions


                rlvFactor[layer] = Math.Max(rlvFactor[layer], 0.0);
                rlvFactorTotal += rlvFactor[layer];
            }

            dltLenghtTotal = ddm_root / sm2smm * specific_root_length;                           // specific root length in range of 40000 - 150000 mm/g 

            for (int layer = 0; layer <= deepest_layer; layer++)
            {
                dltRootLength[layer] = (float)(dltLenghtTotal * (rlvFactor[layer] / rlvFactorTotal));
                root_length[layer] += dltRootLength[layer];
                rlv[layer] = (root_length[layer] / dlayr[layer]);
            }
        ...
        // ====================================================================
        //   methods to calculate soil water values and return ratios and factors
        public double sw_avail_ratio(int layer)
        {
            double fasw = swlayr[layer] * dlayr[layer] / ((dul_layers[layer] - cll[layer]) * dlayr[layer]);
            fasw = Math.Min(1.0, Math.Max(0.0, fasw));               // set limits (0.0 <= fasw1 <= 1.0)
            return fasw;
        }


        public double sw_fac_root(double fasw)
        {
            double swFactor = 1.0;
                // x_sw_ratio = new double[4] { 0.0, 0.25, 0.5, 1.0 };
                // y_swfac_root = new double[4] { 0.0, 0.2, 1.0, 1.0 };
            swFactor = mvDataFunctions.linear_interp_real(fasw, x_sw_ratio, y_swfac_root, x_sw_ratio.Count());
            return swFactor;
        }

        public double sat_fac_root(double satValue)
        {
            double satFactor = 1.0;
            // x_sat_ratio = new double[2] { 1.0, 1.2 };
            // y_satfac_root = new double[2] { 1.0, 0.0 };
            satFactor = mvDataFunctions.linear_interp_real(satValue, x_sat_ratio, y_satfac_root, x_sat_ratio.Count());
            return satFactor;
        }
        ...
        public void ozcot_emerg()
		{
            ...
            double establish;
            //double dltRootDepth_mm;
            double rootGrowthRateDDAdj;
			...
			if (initial_call == true)      // first call of season
			{
                ...
                rootGrowthRateDD = 0.7;      //constant growth of radicle:  0.7 * 60 DD @ 5 cm planting depth =~ 35mm root growth
                rootDepth_mm = sdepth * 10;  //seed depth cm to mm
                initial_root_wt = 1.0;       //estimated root weight at emergence g/m2 (seed @ 11,200 seeds/kilo planted at 10 plants/m2)
            ...
            if ((smi > watlog_c)) nwet_co = nwet_co + 1; 	// count wet days to emergence
            ...
            //root growth (slows if emergence is delayed)
            rootGrowthRateDDAdj = rootGrowthRateDD * (dd_emerg / (dd_emerg + delay_emerg));
            dltRootDepth_mm = rootGrowthRateDDAdj * dd;
            rootDepth_mm += dltRootDepth_mm;
			...
			iemrg = DAS;
            ddmerg = sumdd - dd;      //set dd for emergence to yesterday's sumdd
            cropStage = preSquaring;  //emerged heading towards first square

           
            // initialise root_length array
            Array.Clear(root_length, 0, max_layers);
            Array.Clear(rlv, 0, max_layers);
            double initial_root_length_mm = initial_root_wt/sm2smm * specific_root_length;
            double rld = initial_root_length_mm / (rootDepth_mm * sm2smm);           // (mm/mm3)  should this be length/unit volume soil per plant?  or per sq m?
            int num_root_layers = mvDataFunctions.find_layer_no(rootDepth_mm / 10.0, dlayr_cm, max_layers);
            double cum_depth_cm = 0.0;

            for (int layer = 1; layer <= num_root_layers; layer++)
            {
                if (cum_depth_cm + dlayr_cm[layer] <= (rootDepth_mm / 10.0))
                {
                    root_length[layer] = rld * dlayr[layer] * sm2smm * 1.0 ;
                    rlv[layer] = rld;
                    cum_depth_cm += dlayr_cm[layer];
                }
                else
                {
                    double rooted_proportion = ((rootDepth_mm / 10.0) - cum_depth_cm) / dlayr_cm[layer];
                    root_length[layer] = rld * dlayr[layer] * sm2smm * rooted_proportion;
                    rlv[layer] = rld * rooted_proportion;
                    cum_depth_cm = rootDepth_mm / 10.0;
                }
            }
        ...
        // ====================================================================
        //      subroutine ozcot_fract_interception (skiprow,lai,rowspacing)
        ...
        public void ozcot_fruit()
		{
            ...
            if (useYeatesBPerFunct)
            {
                bper = 0.00122 * tempav - 0.0165;           // boll period Yeates (Ord derived)  (1/days)  added Nov 2015
            }
            else  // use Hearn's standard OZCOT equation
            {
            ...
            //*************************************************************************************** dbj Nov 2015
            // Increase shedding when low minimum temperatures are encountered
            // (Steve Yeates studies from N.Australia)
            //   If low (< 15oC) average min temps are encountered at flowering and for the 2 days post anthesis,
            //   then flowers (now small bolls) will be increasingly shed.
            //   Ignore this shedding if <= BackgroundShedding as this will already have been taken care of on the day of flowering.

            if (minTemps.Average() < 15.0)
            {
                lowMinsSurvivalRate = (-0.0031 * Math.Pow(minTemps.Average(), 2) + 0.1168 * minTemps.Average() - 0.1744);
                if (lowMinsSurvivalRate < 0.0) lowMinsSurvivalRate = 0.0;
                if (lowMinsSurvivalRate > 1.0) lowMinsSurvivalRate = 1.0;
            }
            else
            {
                lowMinsSurvivalRate = 1.0;
            }

            if (lowMinsSurvivalRate < background_retention)
            {
                // need to look back two days and adjust the fruit numbers for the cohorts that were in flower on that day and have 
                // been exposed to the low minimum temperatures.  Note: there may be none, one or more than one cohort.
                listCohortsInFlower.Clear();
                fruitToShed = 0.0;
                int dayKey = (DAS - 2);

                if (floweringDays.ContainsKey(dayKey))
                {
                    listCohortsInFlower.AddRange(floweringDays[dayKey]);

                    //////Debug print details of adjustments
                    ////using (System.IO.StreamWriter adjustmentsfile =
                    //// new System.IO.StreamWriter(@"C:\temp\cohortsToAdjust.txt", true))
                    ////{
                    ////    string mylist = String.Join(",", listCohortsInFlower);
                    ////    string output = String.Format("On {0} DAS, dayKey:{1}  ({2},{3}) : cohorts that were in flower and will be adjusted: {4}.  Calculated Survival Rate: {5}  MinTempAvg: {6}",
                    ////              DAS, dayKey, Today ,DOY, mylist, lowMinsSurvivalRate, minTemps.Average());
                    ////    adjustmentsfile.WriteLine(output);
                    ////}


                    foreach (int day in listCohortsInFlower)
                    {
                        // now need to check the marked fruit in age3 for this day (cohort) and adjust if needed
                        fruitToShed = (1.0 - lowMinsSurvivalRate) * fruno[day];

                        ////using (System.IO.StreamWriter adjustmentsfile2 =
                        //// new System.IO.StreamWriter(@"C:\temp\cohortsToAdjust2.txt", true))
                        ////{
                        ////    string output = String.Format("in adjustment loop for cohort {0}.  Fruit to shed = {1},  fruno[day]={2}; "+
                        ////                                   " frmark[day, age3]={3}; fruno[DAS]={4}", day, fruitToShed, fruno[day], frmark[day, age3], fruno[DAS]);
                        ////    adjustmentsfile2.WriteLine(output);
                        ////}

                        if (fruitToShed > frmark[day, age3])
                        {
                            double oldValue = frmark[day, age3];

                            frmark[day, age3] = fruitToShed;

                            ////using (System.IO.StreamWriter adjustmentsfile2 =
                            //// new System.IO.StreamWriter(@"C:\temp\cohortsToAdjust2.txt", true))
                            ////{
                            ////    string output = String.Format("   Old values {0}  :  New Values {1}", oldValue, frmark[day, age3]);
                            ////    adjustmentsfile2.WriteLine(output);
                            ////}
            ...
            {                              // there are flowers today
                ...
                floweringDays.Add(DAS, listCohortsInFlower);   //add the list of cohorts (usually only one) to the list of what flowered on which days
            }
            ...
            //TODO: ? revise values if fruit counts are being used/set
  ...
  ////          }
        ...
        public void ozcot_istsq()
		{
            ...
            if (useDDsumTo1stSq == true)
            {
                ...
                if (smi < smi_delay_crit)           // water stress delays squaring   :  smi_delay_crit = 0.25
                {
            ...
            //-- new code ------------------------------------------------------------
            {
                ...
                if (smi < smi_delay_crit)           // water stress delays squaring //smi < 0.25
                {
        ...
        public void ozcot_laigen()
        {
            ...
            // TODO: ??  defoliation delay effect needs to be based on thermal time or some other criteria to handle different conditions/regions
            ...
            //  TODO: ??  need leaf detachment logic to return lead to SurfaceOM
       ...
       // public void ozcot_n_fertilise(double applied, double availn, double applied_avail)
        ...
        public void ozcot_solwat()
        {
            ...
            //ozcot_swbal_et();                    // removes ep & es from layers  //replaced by ozcot_removeEt_swbal() Aug 2016
            ozcot_swbal_removeEp();                // removes ep & es from layers using kL values
        ...
        public void ozcot_evapotransp()
		{
			...
			et = es + ep;  //initial setting. Maybe updated in ozcot_removeEt_swbal() if soil is too dry
        ...
        public void ozcot_indices()
        {
            ...
            smi30 = Math.Min(Math.Max(smi_30,0.0),1.0);

            ////debug  Console.WriteLine("DAS= {0} rtsw= {1} rtul= {2} smi_rt= {3} smi30= {4} ", DAS, rtsw.ToString("0.###"), rtul.ToString("0.###"), smi_rt.ToString("0.###"), smi30.ToString("0.###"));
            //if (DAS > 108 && DAS < 120)
            //{
            //    Console.WriteLine("DAS= {0} rtdep= {1} depth= {2} rtul= {3} rtsw= {4} smi= {5} smi_rt= {6} smi_30= {7}", DAS, rtdep, depth, rtul, rtsw, smi, smi_rt, smi_30);
            //}


            //smi = Math.Max(smi_rt, smi_30);

            if ((smi_rt > watlog_c) || (smi30 > watlog_c))
            {
                smi = Math.Min(smi_rt, smi30);  //minimise waterlogging if possible
            }
            else
            {
                smi = Math.Max(smi_rt, smi30);  //minimise any potential water stress
            }
        ...
        //     NOTE: replaced by ozcot_removeEt_swbal()  which uses kL extraction approach
        //{
        //    // ====================================================================
        ...
        // ====================================================================
        public void ozcot_swbal_removeEp()
        {

            // ====================================================================
            //
            //     CALCULATES WATER AVAILABLE FOR EP AND ACTUAL UPTAKE OF WATER BASED
            //     ON kL APPROACH (AFTER PASSIOURA AND MONTIETH).
            //     CALCULATES FOR EACH LAYER AND FOR THE WHOLE PROFILE.
            //     THEN REMOVES TRANSPIRATION (EP) FOR LAYERS.
            // 
            //     sw(L) is modified by extraction, and EP may be modified.
            //
            // ====================================================================


            int maxRootLayer;
            int layerIdx;         // loop counter for layers
            double cumDepth;      // total depth extraction calculated to
            double[] layerSWSupply = new double[10];  // sw by layer
            double supplyFrac;    // fraction of total EP to extracted from given layer
            double epLayerUptake;  // Amount EP will extract from given layer
            double totalSWAvail;  // total sw available in all rooting layers (cm)
            double ep_cm_orig_debug;

            //- implementation section ----------------------------------


            // Check if there is any EP to extract from the soil
            //  (note ES is calculated and extracted by SoilWat component)
            if (ep == 0.0) return;   //Nothing to extract. 


            ep_cm_orig_debug = ep;

            //
            // From the Rooting Depth, find the deepest soil layer containing roots
            //
            cumDepth = 0.0;
            maxRootLayer = 0;

            for (layerIdx = 1; layerIdx <= nlayr; layerIdx++)
            {
                cumDepth = cumDepth + dlayr_cm[layerIdx];
                if ((cumDepth > rtdep) | (layerIdx == nlayr))
                {
                    maxRootLayer = layerIdx;
                    break;
                }
            }

            //
            // Calculate the water available for extraction by the plant from each soil layer
            // and the total available from the root zone of the profile
            //
            totalSWAvail = 0.0;
            Array.Clear(layerSWSupply, 0, layerSWSupply.Length);

            for (layerIdx = 1; layerIdx <= maxRootLayer; layerIdx++)         // For each layer in the profile to the depth of the roots
            {
                layerSWSupply[layerIdx] = swlayr[layerIdx] * dlayr_cm[layerIdx] * kl[layerIdx];   // Water extractable in layer
                if (layerSWSupply[layerIdx] < 0.0) layerSWSupply[layerIdx] = 0.0;

                // Adjust bottom layer for proportion of depth explored by roots
                if (layerIdx == maxRootLayer)
                {
                    layerSWSupply[layerIdx] = layerSWSupply[layerIdx] * ((rtdep - (cumDepth - dlayr_cm[layerIdx])) / dlayr_cm[layerIdx]);
                }

                totalSWAvail = totalSWAvail + layerSWSupply[layerIdx];    // Keep track of the total water in the root zone
            }

            //
            // Calculate the plant uptake from each layer, and from the profile as a total
            //
            if (totalSWAvail <= 0.0)               // No extractable water left in profile
            {
                et = es + 0.0;                     // ensure reported et value refects zero ep value
                storedSWDemandSupplyRatio = 100;   // arbitarily high value to avoid division by zero
                //Console.WriteLine("DAS= {0}    totalSWAvail= {1}  maxRootLayer= {2} rtdep= {3}", DAS, totalSWAvail, maxRootLayer, rtdep.ToString("##0.#"));
                return;                   
            }

            // store demand/supply ratio for use in calc of root growth rate tomorrow (already calculated for today based on yesterday's values).
            storedSWDemandSupplyRatio = ep / totalSWAvail;



            if (ep < totalSWAvail)
            {
                // Demand by crop is less than what roots can provide: water is non - limiting
                // Distribute demand in proportion to capacity of each layer to supply.
                for (layerIdx = 1; layerIdx <= maxRootLayer; layerIdx++)         // For each layer in the profile to the depth of the roots
                {
                    supplyFrac = layerSWSupply[layerIdx] / totalSWAvail;   // Proportion of supply from this layer
                    if (supplyFrac < 0.0) supplyFrac = 0.0;
                    epLayerUptake = supplyFrac * ep;              //Uptake required from layer
                    swlayr[layerIdx] = ((swlayr[layerIdx] * dlayr_cm[layerIdx]) - epLayerUptake) / dlayr_cm[layerIdx];   // Remove water from layer
                }
            }
            else
            {
                // Water supply is insufficient to meet demand: take what is available
                for (layerIdx = 1; layerIdx <= maxRootLayer; layerIdx++)         // For each layer in the profile to the depth of the roots
                {
                    swlayr[layerIdx] = ((swlayr[layerIdx] * dlayr_cm[layerIdx]) - layerSWSupply[layerIdx]) / dlayr_cm[layerIdx];   // Remove water from layer
                }
                // There was insufficient water to supply the expected EP, so adjust EP to the max avail
                ep = totalSWAvail;
            }

            // ensure reported et matches the sum of the es and ep removed
            et = es + ep;


            //Console.WriteLine("DAS= {0} ep_cm_orig_debug= {1} ep_cm= {2} totalSWAvail_cm= {3} rtdep= {4} ", DAS, ep_cm_orig_debug.ToString("##0.###"), (ep).ToString("#0.###"), (totalSWAvail).ToString("#0.###"), rtdep.ToString("##0.#"));
        ...
        public void ozcot_dryxmatter()
        {
            ...
            double fnstrs2;
            double strsbl;
           // double assimilate;  moved to global variable
            double supply;
            double demand;
            double sd_ratio;
            double sd_root;
            ...
            if ((dm_leaf == 0.0))      // leaf weight initialise to zero?
            {
                ...
                //initialise accounting arrays as well
                dm_green[leaf] = dm_leaf;
                dm_green[stem] = dm_stem;
                dm_green[root] = dm_root;
            }
