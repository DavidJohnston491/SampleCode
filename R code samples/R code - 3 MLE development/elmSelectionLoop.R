...
datasetDescr <- " Emerald data with Hybrid columns added (-flwESW) (v4b) "
numberOfTargets <- 6
currentTarget <- 1

reportFile <- "./Reports/ELM_Emer_Hybrid_v04b.rpt"
act_pred_outputDataFile <- "./Reports/Act_Pred_ELM_Emer_Hybrid_v04b.csv"
...
for (loopTarget in targetNames) {
    ...
    for (loopBias in c(TRUE)) {
        #for (loopInitwts in c('normal_gaussian', 'uniform_positive', 'uniform_negative')) {
        for (loopInitwts in c('normal_gaussian')) {    
            #for (loopActFun in c('purelin', 'sig','sin','radbas', 'hardlims','tansig')) {
            for (loopActFun in c('sin')) {
                #for (loopnhid in c(10,100,500,1000)) {
                for (loopnhid in c(200,500,1000)) {
                    ...
                    # name the dataframe
                    names(currentLoopModel) <- c("ModelNo", "Target","nhid","actFun","initWts","bias","MB", "MAE", "RMSE","R2","IOA","COE")
