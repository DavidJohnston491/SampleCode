...
ANN_megainputs <- "ANN_MegaSobolTest22_"
RF_megainputs <- "RF_MegaSobolTest22_"
...
for (loopCnt in (1:length(versions)))  {
  ...
  #^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
  # locations separate
  titleTxt <- paste0("Sobol analysis using 22 machine learning inputs ")
  outputPlot <- paste0("./Reports/Figures/MegaSobol22_" ,versions[loopCnt],"_NoTitle.png")
